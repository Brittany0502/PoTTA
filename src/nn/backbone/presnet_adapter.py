...
class PResNetAdapter(nn.Module):
    def __init__(self,
                 ...
                 mona_dim=128,
                 mona_dropout=0.1,
                 adapter_hidden_ratio=0.125,
                 adapter_use_fft=True,
                 adapter_cutoff_ratio=0.6,
                 adapter_scale_init=0.1,
                 freeze_backbone = False,
                 mona_use_se=True,
                 mona_se_reduction=16
                ):
        super().__init__()
        ...
        self.adapter_hidden_ratio = adapter_hidden_ratio
        self.adapter_use_fft = adapter_use_fft
        ...
        if pretrained:
            ...
            print(f'Load PResNet{depth}+adapter state_dict')
        ...
        for n in range(num_stages):
            ...
            self.adapters_mod0.append(
                MonaFreq2DAdapter(
                    ...
                    se_reduction=mona_se_reduction,
                    use_se=mona_use_se
                )
            )

            if freeze_backbone:
                self.switch_to_stage2()
                print("Backbone frozen, only adapters are trainable")
    ...
    def adapter_named_parameters(self):
        for n, p in self.named_parameters():
            if 'adapters_' in n or 'adapter' in n:
                yield n, p

    def adapter_modules(self):
        return list(self.adapters_mod0)
    ...
    def forward(self, x):
        ...
        for idx, stage in enumerate(self.res_layers):
            ...
            if idx in self.return_idx:
                outs_raw.append(x.clone())

            if self.enable_adapters:
                x = self.adapters_mod0[idx](x)

            if idx in self.return_idx:
                outs.append(x)
                outs_adapt.append(x)

        # ===== [ADD] cache for engine =====
        self._last_outs_raw = outs_raw
        self._last_outs_adapt = outs_adapt
    ...
    def _apply_stage_mode(self, stage2: bool):
        ...
        def set_requires(m: nn.Module, flag: bool):
        ...
        for m in backbone_modules:
            set_requires(m, not stage2)

        for m in self.adapters_mod0 :
            set_requires(m, stage2)
