...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        dec_out_bboxes = []
        dec_out_logits = []
        # 获取每层hidden states，得到 hidden states（content embedding）
        dec_out_hs = []
        ...
        # 形状：
        # dec_out_hs:      [L, B, Nq, C]
        # dec_out_logits:  [L, B, Nq, K]
        # dec_out_bboxes:  [L, B, Nq, 4]

        # print('***********')
        # # 遍历列表，打印每个元素的形状
        # for i, tensor in enumerate(dec_out_hs):
        #     print(f"第 {i} 个元素的形状：", tensor.shape) # [800,300,256]
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits),torch.stack(dec_out_hs)
...
class RTDETRTransformerv2(nn.Module):
    ...
    def _get_decoder_input(self,
        ...
        # 和encoder部分一样，推理过程eval_spatial_size不为None
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_hs = self.decoder(
            init_ref_contents,
        ...
        out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1],
               # ----------增加中间层特征输出---------
               # 新增一行，把所有 decoder 层的输出 logits 当作 queries 返回
               # 形状通常是 [num_layers, batch_size, num_queries, num_classes]
               'hs': out_hs
               }
